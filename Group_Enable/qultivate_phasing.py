...
def expanding_columns():
    ...
    final_df_sheet1.to_excel(block_folder_path + '/' + 'qultivate_phasing.xlsx', sheet_name='qultivate_phasing',
                             index=False)
...
def getting_phasing_csv(folder_path):
    ...
    phasing_command = ['/prj/vlsi/pete/ptetools/prod/utils/tssquery/1.9/tquery.py', '--project', project, '--rev',
                       si_rev, '--block', block_name, '--query', 'phasing_details.query', '--out_path',
                       block_folder_path, '--db', 'BKUP']
...
def populate_skn_data():
    ...
    sku_command = ['/prj/vlsi/pete/ptetools/prod/utils/tssquery/1.9/tquery.py', '--project', project, '--rev', si_rev,
                   '--query', 'sku_definition.query', '--out_path', block_folder_path, '--db', 'BKUP']
...
if __name__ == "__main__":
    ...
    if args.push:
        ...
        filtered_df = modified_df[
            (modified_df['qultivate_enable'].isna()) & (modified_df['qultivate_category'].isna()) & (
                        modified_df['qultivate_value_encoded'] != 0)]
        update_statements = []
        for index, row in filtered_df.iterrows():
            update_values = ', '.join([f"phasing.{col} = {row[col]}" for col in filtered_df.columns if
                                       col != 'phasing_id' and not pd.isna(row[col])])
        ...
        try:
            ...
            conn = pymysql.connect(host=stage_dict['host'], port=stage_dict['port'], user=stage_dict['user'],
                                   passwd=stage_dict['password'], db=stage_dict['db'])
